...
class OSNetSequence(nn.Module):
    ...
    def forward(self, seqs):
        ...
        for image in torch.transpose(seqs, 0, 1):
            image_representations.append(self.extractor(image))
...
class CameraFeedHead(nn.Module):
    def __init__(self, seq_shape, handcraft_feat_len, embed_len=64):
        ...
        self.cnn = LeNetSequence(self.img_shape)
...
class SiameseMultiHeadWrapper(nn.Module):
    def __init__(self, seq_shape, handcraft_feat_len, embed_len, heads=2):
        super(SiameseMultiHeadWrapper, self).__init__()
        self.heads = []
        for _ in range(heads):
            self.heads = CameraFeedHead(seq_shape, handcraft_feat_len, embed_len)
        self.compare = EmbeddingComparisonModel(embed_len)
...
if __name__ == "__main__":
    ...
    test = LeNetSequence(input_shape)
    print(test(rnd_imgs))
    ...
    rnd_feats_plus_time_A = torch.rand(
        (batch_size, handcraft_feat_len + 1), requires_grad=True)

    rnd_seqs_B = torch.rand((batch_size, *seq_shape), requires_grad=True)
    rnd_feats_plus_time_B = torch.rand(
        (batch_size, handcraft_feat_len + 1), requires_grad=True)
